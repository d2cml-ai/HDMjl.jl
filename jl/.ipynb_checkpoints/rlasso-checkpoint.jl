function rlasso(x, y; post = true, maxIter = 15, tol::Float64 = 1e-5, n = size(y, 1), gamma = 0.1 / log(n))
    n = size(y, 1)
    p = size(x, 2)
    ...
    pen = lambdaCalculation(x = x, y = startingval, homoskedastic = true, gamma = gamma)
    lambda = pen["lambda"]
    Ups0 = Ups1 = pen["Ups0"]
    lambda0 = pen["lambda0"]
    ...
    while mm <= maxIter
        ...
        if isempty(x1)
            ...
            coefs = zeros(p + 1, 1)
            coefs = vcat(intercept_value, coefs)
        end
    ...
    global ind1 = ind1
#     global Ups1 = Ups1
#     global lambda0 = lambda0
    
    intercept_value = mu - sum(meanx * coefTemp)
    coefs = vcat(intercept_value, coefTemp)
    s1 = sqrt(var(e1))
    
    est = Dict("coefficients" => coefs, "residuals" => e1, "sigma" => s1, "loadings" => Ups1, "iter" => mm, "lambda0" => lambda0, "lambda" => lambda)
    return est
end
...
#     while mm <= maxIter
#         if mm == 1 && post
#             global coefTemp = LassoShooting_fit(x, y, lambda_half, XX = XX, Xy = Xy)["coefficients"]
#         else
